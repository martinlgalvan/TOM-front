...
// Funciones seguras para acceder a localStorage
const safeGetItem = (key) => {
  try {
    return localStorage.getItem(key);
  } catch (e) {
    console.error(`Error obteniendo el item ${key} de localStorage`, e);
    return null;
  }
};
...
const safeSetItem = (key, value) => {
  try {
    ...
    console.error(`Error guardando el item ${key} en localStorage`, e);
  }
};
...
function App() {
  ...
  useEffect(() => {
    if (isAutenticated && !isIOS) {
      const isCheckboxChecked = safeGetItem('noShowPopup');
  ...
  const handleCheckboxChange = (event) => {
    ...
    safeSetItem('noShowPopup', isChecked.toString());
    ...
    toast.info(
      <div className="row justify-content-center text-center ">
        <div className="col-10">
          <button
            className="row justify-content-center bg-primary rounded-3 "
  ...
  function onLogin(user, token) {
    ...
    safeSetItem('_id', user._id);
    safeSetItem('name', user.name);
    safeSetItem('noShowPopup', 'false');

    safeSetItem('email', user.email);
    safeSetItem('entrenador_id', user.entrenador_id);
    safeSetItem('logo', user.logo);

    if (user.color === undefined) {
      safeSetItem('color', '#1a1a1a');
    } else {
      safeSetItem('color', user.color);
    }

    if (user.color === undefined) {
      safeSetItem('textColor', false);
    } else {
      safeSetItem('textColor', user.textColor);
    }
    navigate(`/`);
  }
    ...
    <>
      ...
      <nav
        className={`navbar navbar-expand-lg navbar-dark fixed-top ${
          textColor === 'false' || !textColor ? "bbb" : "blackColor"
        }`}
      ...
      {/* SIDEBAR */}
      <Sidebar visible={menuSidebar} onHide={handleMenuSidebarHide} blockScroll={true} position="right">
      ...
      <footer className={`container-fluid colorFooter`}>
        <div className={`row marginSidebarClosed`}>
          ...
          <p className="m-0 text-light text-center py-2">&copy; 2022 | TOM</p>
        </div>
      </footer>

      {/* DIÁLOGO DE LOGOUT */}
      <Dialog header="Sesión" visible={openDialogLogout} onHide={() => setOpenDialogLogout(false)}>
      ...
      {isIOS && (
        ...
        >
          <p>
            Para instalar esta aplicación, presiona el botón Compartir de Safari y selecciona
            "Agregar a pantalla de inicio / Agregar a inicio".
          </p>
